...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--tgtemb-dropout', type=float, default=0, metavar='D',
                            help='dropout probability')
        parser.add_argument('--srcemb-dropout', type=float, default=0, metavar='D',
                            help='dropout probability')
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward_embedding(self, src_tokens):
        ...
        if self.args.srcemb_dropout > 0 and self.training:
            droped_embedding = self.embedded_dropout(self.embed_tokens, src_tokens,
                                            self.args.srcemb_dropout, None)
            x = embed = self.embed_scale * droped_embedding
        else:
            x = embed = self.embed_scale * self.embed_tokens(src_tokens)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        if self.args.tgtemb_dropout > 0 and self.training:
            droped_embedding = self.embedded_dropout(self.embed_tokens, prev_output_tokens,
                                            self.args.tgtemb_dropout, None)
            x = self.embed_scale * droped_embedding
        else:
